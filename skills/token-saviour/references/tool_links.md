...
Prerequisites by installer: `serena` needs `uv`; `rtk` needs Homebrew or Cargo unless using the
upstream shell installer; `caveman` and `ponytail` need Node >=18 for some agent targets.

## Defaults

`serena` owns the code-read layer. `rtk`, `caveman`, and `ponytail` each own a distinct layer
and stack cleanly on top.
